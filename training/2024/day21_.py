...
def minimal_path(code_: str, nb_levels_: int) -> str:
    naive_seq = code_to_seq(code_)

    current_pos = numeric_keypad["A"]
    min_path_len = float("inf")
    best = None
    FORBIDDEN_SQUARE = (3, 0)  # the gap area on the directional keypad
    for smart_sequence in optimized_paths_generator(naive_seq):
        seq = ""
...
def solve_p1(codes: list[str], nb_levels: int = 2) -> int:
    ans = 0
    for code_ in codes:
        sequence_ = minimal_path(code_, nb_levels)
...
if __name__ == "__main__":
    ...
    #     )
    #     if len(actual_seq) != len(expected):
    #         print(actual_seq, expected,sep='\n');exit()
