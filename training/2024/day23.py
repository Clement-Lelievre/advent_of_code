...
def solve_p1(inp: str) -> int:
    ...
    players: set[str] = set()
    two_relations = set()
    nb_three_relations = 0
    for line in lines:
        p1, p2 = line.split("-")
        players.add(p2)
        players.add(p1)
        two_relations.add(frozenset([p1, p2]))
    for comb in combinations(players, 3):
        if not any(player.startswith("t") for player in comb):
            continue
        for duo in combinations(comb, 2):
            if frozenset(duo) not in two_relations:
                break
        else:
            nb_three_relations += 1

    print(f"Part 1: {nb_three_relations}")
    return nb_three_relations
