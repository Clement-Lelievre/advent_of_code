...
def solve_p1(inp: str) -> int:
    patterns = {p.strip() for p in inp.strip().splitlines()[0].split(",")}
    min_len_pattern = min(map(len, patterns))
    wanted_designs = inp.strip().splitlines()[2:]

    possible_designs = set()
    seen_remaining_strs = set()

    def recurse(initial: str, remaining: str) -> None:
        nonlocal seen_remaining_strs, stop_all_calls
        if stop_all_calls:
            return
        if initial == remaining:  # new string to try: reset the seen states
            seen_remaining_strs = set()

        if remaining == "":  # the whole string has been made
            possible_designs.add(initial)
            stop_all_calls = True
            return
        if remaining in seen_remaining_strs:
            return
        seen_remaining_strs.add(remaining)
        for i in range(min_len_pattern, len(remaining) + 1):
            if remaining[:i] in patterns:
                recurse(initial, remaining[i:])

    for wanted in wanted_designs:
        stop_all_calls = False  # optim: bool used to stop all incoming recursive calls whenever any of them has made the full string
        recurse(wanted, wanted)

    print(f"Part 1: {len(possible_designs)}")
...
def solve_p2(inp: str) -> int:
    ...
    def count_ways(design: str, cache: dict) -> int:
    ...
    nb_ways = sum(count_ways(wanted, cache={}) for wanted in tqdm(wanted_designs))

    print(f"Part 2: {nb_ways}")
    return nb_ways
